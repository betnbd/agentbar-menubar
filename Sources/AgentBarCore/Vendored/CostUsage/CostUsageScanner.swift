...
enum CostUsageScanner {
    private static let log = AgentBarLog.logger(LogCategories.tokenCost)
        ...
        options: Options = Options()) -> CostUsageDailyReport
    {
        ...
        case .zai, .gemini, .antigravity, .opencode, .opencodego, .copilot, .minimax, .kilo, .kiro, .kimi,
             .kimik2, .jetbrains, .amp, .ollama, .synthetic, .openrouter, .warp, .perplexity:
            return emptyReport
        }
    }
